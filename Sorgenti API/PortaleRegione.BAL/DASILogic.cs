...
namespace PortaleRegione.API.Controllers
{
    public class DASILogic : BaseLogic
    {
        ...
        {
            var attoInDb = await _unitOfWork.DASI.Get(request.UIDAtto);
            if (attoInDb == null) throw new InvalidOperationException("Atto non trovato");
            ...
            try
            {
                ...
                foreach (var idGuid in firmaModel.Lista)
                {
                    ...
                    else
                    {
                        ...
                        {
                            results.Add(idGuid.ToString(),
                                $"ERROR: Il Proponente non ha ancora firmato l'atto {nome_atto}.");
                            continue;
                        }
                    ...
                    if (valida && (atto.IDStato == (int)StatiAttoEnum.PRESENTATO ||
                        ...
                        try
                        {
                            ...
                            _unitOfWork.Stampe.Add(new STAMPE
                            {
                                ...
                                Ordine = 1,
                                Notifica = true,
                                Scadenza = DateTime.Now.AddDays(
                                    Convert.ToDouble(AppSettingsConfiguration.GiorniValiditaLink)),
                                DASI = true
                            });
            ...
            PersonaDto persona)
        {
            ...
            foreach (var idGuid in model.Lista)
            {
                ...
                atto.DataPresentazione = CryptoHelper.EncryptString(
                    atto.Timestamp.Value.ToString("dd/MM/yyyy HH:mm:ss"),
                    AppSettingsConfiguration.masterKey);
        ...
        public async Task<List<AttoDASIDto>> GetMOZAbbinabili(PersonaDto persona)
        {
            ...
            var result = new List<AttoDASIDto>();
            foreach (var seduta in sedute_attive.Where(i =>
                         !sedute_da_escludere.Contains(i.UIDSeduta) && i.Blocco_MOZ_Abbinate == false)) // #1488
            {
            ...
            if (model.Tipo == (int)TipoAttoEnum.RIS)
            {
                for (var i = 0; i < model.Salto; i++)
                    atti_cartacei.Add(new ATTI_DASI
                    {
                        ...
                        DataPresentazione = CryptoHelper.EncryptString(
                            data_presentazione.ToString("dd/MM/yyyy HH:mm:ss"),
                            AppSettingsConfiguration.masterKey),
        ...
        public async Task SalvaCartaceo(AttoDASIDto attoDto, PersonaDto currentUser)
        {
            ...
            if (attoDto.IsRIS() && !attoInDb.Progressivo.Equals(attoDto.Progressivo))
            {
                ...
                if (controlloEtichetta != null) throw new InvalidOperationException("Numero atto già occupato.");
            }
        ...
        public async Task CreateExcelSituazioneReport(string filePath, ReportDto model, PersonaDto currentUser)
        {
            ...
            if (!string.IsNullOrEmpty(tipi_join)) titolo_report += $" {tipi_join}";

            if (!string.IsNullOrEmpty(date_join)) titolo_report += $"{date_join}";
        ...
        private async Task<object> GetPropertyValue(AttoDASIDto atto, string propertyName,
            ExportFormatEnum exportFormat)
        {
            ...
            if (propertyName.Equals(nameof(AttoDASIReportDto.Iniziativa)))
            {
                ...
                if (atto.FirmeAnte.Any(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                {
                    firme.AddRange(atto.FirmeAnte.Where(f => string.IsNullOrEmpty(f.Data_ritirofirma))
                        .Select(f => f.FirmaCert));

                    if (atto.FirmePost.Any(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                        firme.AddRange(atto.FirmePost.Where(f => string.IsNullOrEmpty(f.Data_ritirofirma))
                            .Select(f => f.FirmaCert));
                }
            ...
            if (propertyName.Equals(nameof(AttoDASIReportDto.Firme_DCR)))
            {
                ...
                if (atto.FirmeAnte.Any(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                {
                    foreach (var attiFirmeDto in atto.FirmeAnte.Where(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                    {
                    ...
                    if (atto.FirmePost.Any(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                    {
                        foreach (var attiFirmeDto in atto.FirmePost.Where(f => string.IsNullOrEmpty(f.Data_ritirofirma)))
                        {
                            var firmatario = await _unitOfWork.Persone.Get(attiFirmeDto.UID_persona);
                            firme.Add(firmatario.cognome.ToUpper());
                        }
            ...
            if (propertyName.Equals(nameof(AttoDASIDto.Risposte)))
            {
                ...
                var risposteList = new List<string>();
                    ...
                    if (atto.IDTipo_Risposta_Effettiva.HasValue)
                    {
                        ...
                        switch ((TipoRispostaEnum)atto.IDTipo_Risposta_Effettiva)
                        {
                            ...
                            case TipoRispostaEnum.ORALE:
                                risposta = $"Orale: Risposta fornita da {attiRisposteDto.DescrizioneOrgano}";
                                break;
                            case TipoRispostaEnum.SCRITTA:
                                risposta = $"Scritta: Risposta fornita da {attiRisposteDto.DescrizioneOrgano}";
                                break;
                            ...
                            case TipoRispostaEnum.ITER_IN_ASSEMBLEA:
                                risposta = "Atto trattato in assemblea";
                                break;
                            case TipoRispostaEnum.ITER_IN_ASSEMBLEA_COMMISSIONE:
                                risposta = $"{attiRisposteDto.DescrizioneOrgano}";
                                break;
                            default:
                                risposta =
                        ...
                        if (attiRisposteDto.Data.HasValue)
                        {
                            ...
                            if (exportFormat == ExportFormatEnum.PDF || exportFormat == ExportFormatEnum.WORD)
                                risposta += $" - <b>Data risposta:</b> {dataRisposta}";
                            else
                                risposta += $" - Data risposta: {dataRisposta}";
                        }
                        ...
                        if (attiRisposteDto.DataTrasmissione.HasValue)
                        {
                            var dataRispostaTrasmissione =
                                attiRisposteDto.DataTrasmissione.Value.ToString("dd/MM/yyyy");
                            if (exportFormat == ExportFormatEnum.PDF || exportFormat == ExportFormatEnum.WORD)
                                risposta += $" - <b>Trasmessa il:</b> {dataRispostaTrasmissione}";
                            else
                                risposta += $" - Trasmessa il: {dataRispostaTrasmissione}";
                        }

                        risposteList.Add(risposta);
                    }

                var separatore = exportFormat == ExportFormatEnum.PDF || exportFormat == ExportFormatEnum.WORD
                    ? ";<br>"
                    : "; ";
                bodyRisposte = string.Join(separatore, risposteList);
            ...
            if (propertyName.Equals(nameof(AttoDASIDto.Note)))
            {
                ...
                if (note.Any()) return note.Select(a => $"{a.Tipo} - {a.Nota}").Aggregate((i, j) => i + ", " + j);
            ...
            // #1021
            if (propertyName.Equals(nameof(AttoDASIDto.Legislatura))) return atto.GetLegislatura();
        ...
        private async Task<string> ComposeReportBodyFromTemplate(ReportDto model, PersonaDto currentUser)
        {
            ...
            switch ((DataViewTypeEnum)model.dataviewtype)
            {
                ...
                case DataViewTypeEnum.CARD:
                    ...
                    {
                        var atto = await GetAttoDto(guid, currentUser);
                        body += templateItemCard_Standard.Replace("{{ITEM}}",
                            await GetBodyItemCard(atto, model.columns));
                    }
        ...
        public async Task<AttiDocumentiDto> Salva_Documento(SalvaDocumentoRequest request, PersonaDto currentUser)
        {
            ...
            if (!string.IsNullOrEmpty(atto.Etichetta)) dir += $"/{atto.Etichetta}";
        ...
        public async Task Salva_ComandoMassivo(SalvaComandoMassivoRequest request, PersonaDto currentUser)
        {
            foreach (var datiInline in request.Lista)
            {
                ...
                if (request.Dati.EmendatoCheck) atto.Emendato = request.Dati.Emendato;

                if (request.Dati.PubblicatoCheck) atto.Pubblicato = request.Dati.Pubblicato;
