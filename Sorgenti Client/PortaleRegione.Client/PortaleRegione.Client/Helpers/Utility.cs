...
namespace PortaleRegione.Client.Helpers
{
    ...
    public static class Utility
    {
        ...
        public static void AddFilter_ByOggetto_Testo(ref BaseRequest<AttoDASIDto> model, string filtroOggetto)
        {
            ...
            string filtroNAtto2)
        {
            ...
            {
                int number;
                var success = int.TryParse(filtroNAtto, out number);
                if (success && number > 0)
                {
                    var operation = Operation.EqualTo;
                    int number2;
        ...
        public static void AddFilter_ByLegislatura(ref BaseRequest<AttoDASIDto> model, string filtroLegislatura)
        {
            if (!string.IsNullOrEmpty(filtroLegislatura))
            {
                int number;
                var success = int.TryParse(filtroLegislatura, out number);
                if (success && number > 0)
                    model.filtro.Add(new FilterStatement<AttoDASIDto>
                    {
        ...
        public static void AddFilter_ByDataPresentazione(ref BaseRequest<AttoDASIDto> model, string filtroDa,
            string filtroA)
        {
            if (!string.IsNullOrEmpty(filtroDa))
            {
                ...
                model.filtro.Add(new FilterStatement<AttoDASIDto>
                {
                    PropertyId = nameof(AttoDASIDto.Timestamp),
                    Operation = operation,
                    Value = Convert.ToDateTime(filtroDa).ToString("yyyy-MM-dd") + " 00:00:00",
                    Connector = FilterStatementConnector.And
                });
            }
        }
